...
def multinomial_num_samples_1(probs: torch.Tensor) -> torch.Tensor:
    if torch._dynamo.is_compiling():
        ...
        return torch.argmax(probs / distribution, dim=-1, keepdim=True), torch.max(probs, dim=-1, keepdim=True)
    i = torch.multinomial(probs, num_samples=1)
    return i, probs[i]
...
def sample(
    ...
    return torch.argmax(logits, dim=-1, keepdim=True), torch.max(logits)


def next_token(model: GPT, input_pos: torch.Tensor, x: torch.Tensor, **kwargs: Any) -> torch.Tensor:
    logits = model(x, input_pos)
    _next, _prob = sample(logits, **kwargs)[0].to(dtype=torch.int64), sample(logits, **kwargs)[1]
    return _next, _prob
...
def generate_fn(
    ...
    *,
    ...
    for current_idx in range(max_returned_tokens - prompt_size):
        ...
        # Generate the token
        token, prob = next_token(model, input_pos, token.view(1, -1), temperature=temperature, top_k=top_k, top_p=top_p)
        # print("prob = ", prob)
        total_prob *= prob
        tokens.append(token)
